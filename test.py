...
# fonction pour recup les liens categories
def get_category_link(base_url):
    response = requests.get(base_url)
    soup = BeautifulSoup(response.content, 'lxml')

    # pour recup les categories
    nav_menu = soup.find('div', class_='side_categories')
    categories = nav_menu.find_all('a')
    link_category = []
...
# boucle pour parcourir les pages
while next_page_url:
    # Obtenir les liens de catégorie pour chaque page
    category_links = get_category_link(next_page_url)

    # Boucle sur chaque catégorie
    for category_link in category_links:
        # Obtenir le contenu HTML de la page actuelle et beautiful pour analyser l'HTML
        response_category = requests.get(category_link)
        soup_category = BeautifulSoup(response_category.content, 'lxml')

        # Récupération des liens des livres sur la page actuelle
        book_links = soup_category.find_all('h3')
